...
public final class LineRequest {
    ...
    private void validateName(final String name) {
        if (name == null || name.isBlank()) {
            throw new SubwayException("노선의 이름은 최소 한 글자 이상이어야 합니다.");
        }
        if (name.length() > MAX_NAME_LENGTH) {
            throw new SubwayException("노선의 이름은 " + MAX_NAME_LENGTH + "자를 넘을 수 없습니다.");
        }
    }

    private void validateUpStationId(final long upStationId) {
        if (upStationId < 0) {
            throw new SubwayException("상행역의 id는 음수가 될 수 없습니다.");
        }
    }
    ...
    private void validateColor(final String color) {
        if (color == null || color.isBlank()) {
            throw new SubwayException("노선의 색깔은 최소 한 글자 이상이어야 합니다.");
        }
        if (color.length() > MAX_COLOR_LENGTH) {
            throw new SubwayException("색깔 문자열의 길이는 " + MAX_COLOR_LENGTH + "자를 넘을 수 없습니다.");
        }
    }
