...
- [ ] 구간 삭제 기능 심화
  - [ ] 지하철 노선에 등록된 하행 종점역을 삭제한다.
    - [ ] 하행 종점역이 삭제되는 경우, 그 이전 역이 하행 종점역이 된다. (삭제되는 역의 구간의 UpStation)
    - [ ] 구간이 유지될 수 없으므로, 해당 하행 종점역을 포함하는 구간 또한 삭제된다.
  - [ ] 지하철 노선에 등록된 상행 종점역을 삭제한다.
    - [ ] 상행 종점역이 삭제되는 경우, 그 다음 역이 상행 종점역이 된다. (삭제되는 역의 구간의 DownStation)
    - [ ] 구간이 유지될 수 없으므로, 해당 상행 종점역을 포함하는 구간 또한 삭제된다.
  - [ ] 지하철 노선에 등록된 중간 역을 삭제한다.
    - [ ] 중간역이 제거되는 경우, 재배치를 진행한다.
    - [ ] 재배치되는 역들의 거리는, 제거된 역을 포함하는 두 구간의 거리의 합으로 한다.
  - [ ] 예외 케이스
    - [ ] 노선에 등록되어 있지 않은 역을 제거하려는 경우 예외를 던진다.
    - [ ] 구간이 하나인 노선에서, 상행 종점역, 하행 종점역을 제거하려는 경우 예외를 던진다.
